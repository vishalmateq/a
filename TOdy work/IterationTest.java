...
class EventAgency {
	public static Magician getMagician(String hint) {
		...
		else if(hint.equalsIgnoreCase("Human"))
		{
			magician = new HumanMagician();
		}
		return magician;
	}
}
...
class HumanMagician implements Magician
{
	public Magic doTrick(Account accRef) {
		System.out.println("Cutting human in two parts....");
		Magic magic = new HumanMagic();
		accRef.deposit(7000);
		return magic;
	}
}
...
class HumanMagic implements Magic
{
	public void show() {
		System.out.println("Sperating human in two parts ...in a magical way....");
	}
}
